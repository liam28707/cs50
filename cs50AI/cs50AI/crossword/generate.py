...
class CrosswordCreator:
    ...
    def enforce_node_consistency(self):
        ...
        """
        domain_cpy = copy.deepcopy(self.domains)
        for i in domain_cpy:
            length = i.length
            for x in domain_cpy[i]:
                if len(x) != length:
    ...
    def revise(self, x, y):
        ...
        xop, yop = self.crossword.overlaps[x, y]
        bool = False
        if xop is None:
            return False
        elif xop:
            for xwrd in dom_copy[x]:
                match = False
                for ywrd in dom_copy[y]:
                    if xwrd[xop] == ywrd[yop]:
                        match = True
                        break
                if match:
                    continue
                else:
                    self.domains[x].remove(xwrd)
                    bool = True
        return bool
    ...
    def consistent(self, assignment):
        ...
        for variable in assignment:
            ...
            if len(word) != variable.length:
                return False
            for i in self.crossword.neighbors(variable):
                overlap = self.crossword.overlaps[variable, i]
                if overlap is not None:
                    x, y = overlap
                    if i in assignment and word[x] != assignment[i][y]:
                        return False
        return True
            ...
            for j in self.crossword.neighbors(var):
                ...
                if overlap is not None:
                    ...
                    for k in self.domains[j]:
                        if i[position1] != k[position2]:
                            count += 1
            ordered.append((count, i))
        ordered.sort()
        return [value for count, value in ordered]
